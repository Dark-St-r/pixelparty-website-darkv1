...
import { buyState } from "../../utils/store";

const BuyModal = () => {
  const [buyModal, setBuyModal] = useRecoilState(buyState);

  function buyFrame() {
    //console.log(buyModal.frameId +" | "+ buyModal.price);
    NearUtils.buyFrame(buyModal.frameId, buyModal.price);
  }
      ...
      <div className="fixed flex flex-row h-48 rounded-md shadow-xl bg-theme-normal">
        <div className="p-6">
          <span className="mb-2 text-2xl font-semibold text-white ">
            Do you want to buy Frame #{buyModal.frameId + 1}?
          </span>
          <br></br>
          <span className="font-semibold text-gray-500 text-md ">
            This Frame will cost you{" "}
            <span className="px-2 font-bold text-gray-200 bg-theme-dark">
              {buyModal.price} NEAR
            </span>
          </span>
